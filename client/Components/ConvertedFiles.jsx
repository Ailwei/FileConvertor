...
const ConvertedFiles = () => {
  ...
  useEffect(() => {
    const fetchFiles = async () => {
      try {
        ...
        const response = await axios.get('https://51.21.127.210/auth/files', {
          headers: {
    ...
    const fetchStorage = async () => {
      try {
        ...
        const response = await axios.get(`https://51.21.127.210/auth/update-storage/${userId}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const handleSaveClick = async (file) => {
    try {
      const token = sessionStorage.getItem('authToken');
      await axios.put(
        `https://51.21.127.210/auth/updatefiles/${file.filename}`,
        { newFilename },
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleDeleteClick = async (file) => {
    try {
      const token = sessionStorage.getItem('authToken');
      await axios.delete(`https://51.21.127.210/auth/delete/${file.filename}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
            ...
            <ul className="list-group">
              {currentFiles.map((file) => (
                <li key={file.filename} ref={(el) => (fileRefs.current[file.filename] = el)} className="list-group-item">
                  ...
                  <div>
                    <a href={`https://51.21.127.210/auth/download/${file.filename}`} target="_blank" rel="noopener noreferrer" className="btn btn-info btn-sm me-2">
                      View File
                    </a>
