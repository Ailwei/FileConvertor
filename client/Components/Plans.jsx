...
const Plans = () => {
      ...
      try {
        if (decodedToken) {
          const response = await axios.get(
            `http://localhost:3000/auth/current-subscription/${decodedToken.userId}`
          );
  ...
  return (
    <div className="container mt-5">
      <h2 className="mb-4 text-center">Select Your Plan</h2>
      <div className="row">
        {plans.map((plan) => (
          <div className="col-md-4 mb-12 d-flex" key={plan.planType}>
            <div className="card shadow-sm w-100 h-100 d-flex flex-column">
              <div className="card-body d-flex flex-column">
                <h5 className="card-title">{plan.name}</h5>
                <p className="card-text">{plan.description}</p>
                <p className="card-text">{plan.detail}</p>
                {plan.detail1 && <p className="card-text">{plan.detail1}</p>}
                <p className="card-text">{plan.detail2}</p>
                <p className="card-text mt-auto fw-bold">
